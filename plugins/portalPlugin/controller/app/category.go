 ...
 * @Author return <1140444693@qq.com>
 * @Description // 获取顶级分类
 * @Date 2021/1/21 17:31:49
 * @Param
 * @return
 **/

func  (rest *Category) GetTopList(c *gin.Context) {

	category := model.PortalCategory{}
	data, err := category.TopList()
	if err != nil {
		rest.rc.Error(c, err.Error(), nil)
		return
	}
...
func (rest *Category) Show(c *gin.Context) {
	...
	data,err := new(model.PortalCategory).Show([]string{"id = ? and delete_at = ?"},[]interface{}{rewrite.Id, 0})
	if err != nil {
		rest.rc.Error(c,err.Error(),nil)
		return
	}
